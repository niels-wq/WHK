...
if (SMTP_USER && SMTP_PASS) {
  ...
  console.log(`E-mail geconfigureerd via ${SMTP_HOST} → ${NOTIFICATION_EMAIL}`);
} else {
  console.log('E-mail niet geconfigureerd — stel SMTP_USER en SMTP_PASS in als Railway variabelen.');
}
...
async function sendLeadEmail(lead) {
    ...
    <div style="font-family:Arial,sans-serif;max-width:560px;margin:0 auto;">
      ...
      <div style="background:#f7f3ea;padding:20px 24px;border:1px solid #D7CBB0;border-top:none;border-radius:0 0 8px 8px;">
        <table style="width:100%;border-collapse:collapse;font-size:0.9rem;">
          ...
          <tr><td style="padding:8px 0;color:#666;"><strong>Vraag / bericht</strong></td><td style="padding:8px 0;">${lead.message || lead.summary || '—'}</td></tr>
  ...
  try {
    await mailer.sendMail({
      from: `"werkhervattingskas.nl" <${SMTP_USER}>`,
      to: NOTIFICATION_EMAIL,
      subject: `🔔 Nieuwe lead: ${lead.name || 'Anoniem'} via ${bron}`,
      html
    });
...
app.get('/health', async (req, res) => {
  try { await pool.query('SELECT 1'); res.json({ ok: true, db: 'connected', email: !!mailer }); }
...
app.listen(PORT, () => {
  ...
  console.log(`E-mail: ${mailer ? 'ACTIEF' : 'NIET geconfigureerd'}`);
});
